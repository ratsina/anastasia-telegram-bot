...
Интеграция принимает события Instagram Direct и отвечает только на кодовое слово `ДВИЖЕНИЕ`. В Instagram пользователь получает комплекс №1, а за комплексом №2 переходит по deep link в Telegram-бота.
...
- `INSTAGRAM_COMPLEX_1_URL` — публичная ссылка на комплекс №1; до заполнения выводится заглушка.
...
3. В Direct должны появиться кнопки комплекса №1 и получения комплекса №2.
4. Первая кнопка отправляет ссылку на комплекс №1.
5. Вторая кнопка отправляет deep link, который открывает в Telegram этап получения комплекса №2.
